...
mark_ii <- function(x, y, eta, reps=1) {
  ...
  for (j in 1:reps){
    ...
    for (i in sample(1:length(y),replace=F)) { 
      ...
      delta_w22 <- eta * (-1) * (y[i] - ypred22) * ypred1 # d(Loss)/dw_22
      delta_w21 <- eta * (-1) * (y[i] - ypred21) * ypred1 # d(Loss)/dw_21
      delta_w1 <- eta * (y[i] - ypred21 - ypred22) * -1 * (sum(w1*w21) + sum(w1*w22)) #d(Loss)/dw_1
