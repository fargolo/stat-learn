...
Using trigonometric identities, we show that the real part of complex number $s$ in $\zeta(s)$ is bounded by a ratio to to its real part after rotation for infinite non-trivial zeros. Then, we can define its identity by studying symmetries in torsion elements, which are projective line invariants. Specifically, the six cross-ratios possible among four points defining a harmonic range.

These simmetries are the exceptional orbits corresponding to fixed points on the Riemann sphere under the action of the symmetric group $S_{3}$. In this manner, we obtain sets $\{0,1,\infty\}$, $\{\phi_{-},\phi_{+}\}$ and $\{-1,\frac{1}{2},2\}$. The later is the orbit of the harmonic cross-ratio, where $\frac{1}{2}$ is the fixed point of the 2-cycle with linear fractional transform $1-\lambda$. This parameterizes $Re(\zeta(s))$, hence defining the critical line $Re(s)=\frac{1}{2}$ as the location of $Re(\not {Tr_{0}}$.  
