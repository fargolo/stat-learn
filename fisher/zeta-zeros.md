...
$M_{n}$ defines a injective (one-to-one) relation between the sets $S_{n}$ of $\zeta$ spirals and the lines $Re(s)$ parallel to the imaginary axis. 


Those are defined in a way that 
$Re(s) \rightarrow M_{n}$
$M_{(Re(s))} = \zeta(s)$.
