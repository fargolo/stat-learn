...
$Y(\theta) = r(sin \theta - (\theta-\alpha) cos(\theta)).$  
...
In the set $S$ of spirals $R$, consider a potentially non empty subset $S'$ of curves whose involutions define arcs constinously rotated towards the non-trivial values of $Re(\zeta(s))=0$. That is, those with real part not in the set of negative even intergers $Re(s) \notin Tr_{\zeta_{0}}, Re(Tr_{\zeta_{0}}) = -2n, n =1,2,3,4,...$.  

We demonstrate that the $\epsilon$ distance to the spiral's attractor must converge to 0 at a periodic rate generated by the harmonic series. 

**Lemma 2: The unique set of orbits with images containing infinite non-trivial zeros**
