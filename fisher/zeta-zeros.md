...
**Gedankenexperiment**: If we consider that the result of the infinite vectorial operations should periodically be 0, we can mirror the previous example by taking $\phi$ as the ratio between the vector $\vec{a}$ and its real projection part after rotations. Hence, the accumulated differences must periodically converge to zero as we add successive vectors rotated by $\theta$. 

For the projection to be equal to 0, $cos(\theta) = 0, \theta = \frac{\pi}{2}$.
...
$$r = \sqrt{2}, \theta = \frac{\pi}{2} + \frac{\pi}{4}$$
