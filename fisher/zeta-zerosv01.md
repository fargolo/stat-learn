...
In the set of orbits $M_{n} (s)$, consider a potentially non empty set of spirals $(S_{0})$ whose arcs constinously rotate as a function of $s$ towards non-trivial values of $Re(\zeta(s))=0$.  
...
### $S$ Spirals
Define mappings $M$ of the set $Z = \zeta(s)$ in the complex plane to the set $S$ of spirals. 
Each spiral $R$ is a subset of S whose elements are a sequence of involutes $(C_{a}(C + Im(s)))$ of circles $(C cos(\theta), C sin(\theta))$ with identical radius $C_(s)=Re(s)$.

The arc's length equals to $r/2*theta*t_{2}^2$, $\theta \in [t{1},t_{2}]$.
...
Each $M$ map defines a injective (one-to-one) relation between the subsets $R$ of $S$ and the lines parallel to the imaginary axis.  
$$R_{C}(\theta) = \{x|x = C + \theta i\}, C \in \mathbb{R}$$

For standard involutes in the complex plane with vector parameterization $r cos(\theta), r(sin)(\theta)$, $\vec{c}'(\theta) = (-r(sin(\theta), r cos(\theta)$. Therefore, $|\vec{c}'(t)=r|$elements of subset $R(Re(s))$ correspond to increasing arc lenghts as a consequence of rotation by $\theta i$.  
...
### Mertens' theorem
Let (an)n >= 0 and (bn) n>= 0 be real or complex sequences. It was proved by Franz Mertens[citation needed] that, if the series 
